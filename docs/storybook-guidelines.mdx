...
Standards for writing stories for Mirai UI components.
...
- Prefer `args` for props; keep custom `render` simple.
- Show semantic combinations (`variant` × `colorScheme` × `size`) that users actually need.
- Keep layout helpers inside stories minimal (`flex`, `gap`, `w-*`).
